...
def home(request):
    ...
    licenciados = Oficialato.objects.filter(ativo=True).filter(cargo_atual__in=['licenciado'])
    ...
    return render(request, 'home.html', {'progresso_membresia':progresso_membresia,'freq_anual': freq_anual,'freq': freq,'turmas':turmas,'domingos':domingos,'turmas_i':turmas_i,'domingos_i':domingos_i, 'pastores':pastores, 'licenciados':licenciados, 'presbiteros':presbiteros,'diaconos':diaconos})
...
def relatorios(request):
    ...
    sem_casa = pessoas.filter(Q(chefe_familia_id = 888)|Q(chefe_familia_id__isnull = True))
...
def rol_publico(request,tipo='todos'):
    ...
    if(tipo == 'homens'):
        ...
        pessoas = pessoas.filter(Q(chefe_familia_id = 888)|Q(chefe_familia_id__isnull = True))
    elif(tipo == 'solteiros'):
